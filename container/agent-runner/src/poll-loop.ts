...
import { clearContainerProgress } from './mailbox/sqlite/connection.js';
  ...
  try {
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        midTurnTail = '';
        // Live progress: clear the per-turn recent-tools / thinking-line fields
        // so the next turn doesn't open showing the previous turn's progress.
