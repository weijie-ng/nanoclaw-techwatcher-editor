...
describe('defaultResolveRemote', () => {
  ...
  it('does not mistake a lookalike org for the canonical repo', () => {
    // A false canonical match here would surface as the wrong remote rather than
    // the upstream fallback. The remote URL reproduces the exact `.../notnanocoai/
    // nanoclaw.git` boundary the regex must reject, but as a LOCAL bare repo with
    // no `channels` branch, so the has-branch scan stays offline and instant.
    const base = mkdtempSync(join(tmpdir(), 'nc-look-'));
    const fake = join(base, 'notnanocoai', 'nanoclaw.git');
    mkdirSync(join(base, 'notnanocoai'), { recursive: true });
    execSync(`git init -q --bare ${fake}`);
  ...
  it('adds upstream when nothing carries the branch, rather than returning a remote that lacks it', () => {
    ...
    // `return origin` fell through to a fetch that could only fail. A local bare
    // repo with no branches stands in for that lone remote, keeping this offline.
    const forkClone = mkdtempSync(join(tmpdir(), 'nc-fork-'));
    execSync('git init -q --bare', { cwd: forkClone });
    git(`remote add origin ${forkClone}`);
    expect(defaultResolveRemote('channels', root)).toBe('upstream');
    expect(git('remote get-url upstream').trim()).toBe('https://github.com/nanocoai/nanoclaw.git');
    rmSync(forkClone, { recursive: true, force: true });
  });
