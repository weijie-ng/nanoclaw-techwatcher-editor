...
export function defaultResolveRemote(branch: string, root: string): string {
  ...
  const cap = (cmd: string): string => {
    try {
      // GIT_TERMINAL_PROMPT=0 so a remote requiring auth (e.g. a fork clone
      // scanning a private or nonexistent registry URL) fails fast instead of
      // hanging the whole install on git's /dev/tty credential prompt.
      return execSync(cmd, {
        cwd: root,
